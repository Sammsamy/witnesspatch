...
function normalizedRunInput(runInput) {
  return {
...
function selectFailedActionInvariant(caseData, evaluation, targetRuleId) {
...
function buildRegressionTestSource() {
...
export function compileWitnessBundle(caseInput, runInput, options = {}) {
  ...
  const normalizedRun = normalizedRunInput(runInput);
  ...
  const selectionPolicy = options.targetRuleId
  ...
  const staticWitness = assertCompiledWitnessSchema({
    ...minimized,
  ...
  const receipt = {
    ...
    selected_rule_reason: options.targetRuleId
  ...
  const files = new Map([
    ["case.json", jsonFile(caseData)],
    ["run.json", jsonFile(normalizedRun)],
    ["evaluated-run.json", jsonFile(evaluatedRun)],
    ["failing-prefix.json", jsonFile(failingPrefix)],
    ["static-witness.json", jsonFile(staticWitness)],
    ["regression.json", jsonFile(regression)],
    ["regression.test.mjs", buildRegressionTestSource()],
    ["receipt.json", jsonFile(receipt)]
  ]);
  const manifest = assertWitnessBundleManifestSchema({
    schema_version: "1.0.0",
