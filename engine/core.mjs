...
import { assertCaseSchema, assertRunSchema } from "./schema-validator.mjs";
...
function assertExactKeys(value, expectedKeys, label) {
  ...
  requireValue(
    stableStringify(actualKeys) === stableStringify(sortedExpected),
...
export function assertCase(caseData) {
  ...
  for (const rule of caseData.rules) {
    ...
    requireValue(
      unique(metricTags).length === metricTags.length,
      `${rule.id} metric_tags must be unique.`
    );
    for (const tag of metricTags) {
      requireValue(
        ALLOWED_METRIC_TAGS.has(tag),
        `${rule.id} uses unsupported metric tag ${tag}.`
      );
    }
  ...
  if (latencyConfig) {
    requireValue(
      isObject(caseData.metrics) && isObject(latencyConfig),
      "safe_action_latency must be an object."
    );
    requireValue(
    ...
    requireValue(
      actions.has(latencyConfig.action),
      `safe_action_latency references unknown action ${latencyConfig.action}.`
    );
    requireValue(
      taggedLatencyRules.length > 0,
      "safe_action_latency requires at least one tagged trigger rule."
    );
    ...
    requireValue(
      taggedLatencyRules.length === 0,
      "safe_action_trigger tags require a safe_action_latency configuration."
    );
  }

  const controlKinds = caseData.controls.map((control) => control.kind);
  requireValue(
    unique(controlKinds).length === controlKinds.length,
    "Control kinds must be unique."
  );
  requireValue(
  ...
  for (const control of caseData.controls) {
    if (control.kind === "temporal_integrity") {
      requireValue(
        Array.isArray(control.future_fact_markers) &&
          control.future_fact_markers.length > 0,
      ...
      for (const contract of control.future_fact_markers) {
        ...
        requireValue(
          !markedFacts.has(contract.fact_id),
          `${control.id} repeats marked fact ${contract.fact_id}.`
        );
        markedFacts.add(contract.fact_id);
        assertMarkerList(
          contract.markers,
          `${control.id} marker contract for ${contract.fact_id}`
        );
    ...
    if (control.kind === "message_action_consistency") {
      assertMarkerList(
        ...
        `${control.id} global forbidden marker list`
      );
      requireValue(
        Array.isArray(control.action_contracts) &&
          control.action_contracts.length > 0,
      ...
      for (const contract of control.action_contracts) {
        ...
        requireValue(
          !contractedActions.has(contract.action),
          `${control.id} repeats contracted action ${contract.action}.`
        );
        contractedActions.add(contract.action);
        assertMarkerList(
          contract.required_any_markers,
          `${control.id} marker contract for ${contract.action}`
        );
      }
      requireValue(
...
export function assertCandidate(caseData, candidate) {
  ...
  requireValue(
    Array.isArray(candidate.decisions) && candidate.decisions.length > 0,
    "Candidate must contain decisions."
  );
  requireValue(
  ...
  for (const [index, decision] of candidate.decisions.entries()) {
    ...
    requireValue(
      decision.at_minute === step.at_minute,
      `Decision ${decision.step_id} must use the step's time lock (${step.at_minute}).`
    );
    requireValue(
      decision.clinical === true,
      `${decision.step_id} must set clinical to true; healthcare decisions cannot self-exempt from source linkage.`
    );
...
function assertRepairMetadata(runInput) {
  ...
  requireValue(
    (runInput.variant === "repaired") === hasRepair,
    "Only repaired runs may declare repair metadata, and every repaired run must declare it."
  );
  if (!hasRepair) return;
  ...
  requireValue(
    repair.parent_run_id !== runInput.run_id,
    "Repair parent_run_id must differ from the repaired run_id."
  );
  ...
  requireValue(
    normalizeLexicalText(repair.policy_diff.before) !==
      normalizeLexicalText(repair.policy_diff.after),
  ...
  requireValue(
    Array.isArray(repair.regression_assertions) &&
      ...
      repair.regression_assertions.every(
        (assertion) => typeof assertion === "string" && assertion.trim().length > 0
      ) &&
...
export function assertRunInput(caseData, runInput) {
  assertCase(caseData);
  assertCandidate(caseData, runInput);
  requireValue(
    typeof runInput.run_id === "string" && runInput.run_id.trim().length > 0,
    "Run input needs a non-empty run_id."
  );
  requireValue(
    RUN_VARIANTS.has(runInput.variant),
    "Run input variant must be baseline, repaired, or candidate."
  );
  ...
  const unknownProvenanceKeys = Object.keys(runInput.provenance)
    ...
    .sort();
  requireValue(
    unknownProvenanceKeys.length === 0,
    `Run provenance contains unknown keys: ${unknownProvenanceKeys.join(", ")}.`
  );
  requireValue(
  ...
  requireValue(
    typeof runInput.provenance.generation_mode === "string" &&
      runInput.provenance.generation_mode.trim().length > 0,
    "Run provenance needs generation_mode."
  );
  requireValue(
    typeof runInput.provenance.model_invocation_logged === "boolean",
    "Run provenance needs model_invocation_logged boolean."
  );
  requireValue(
    runInput.provenance.contains_real_patient_data === false,
    "Run inputs must explicitly declare that they contain no real patient data."
  );
  requireValue(
    runInput.provenance.clinician_validation === "pending",
    "Run inputs must keep clinician_validation pending."
  );
  requireValue(
    typeof runInput.provenance.api_key_required === "boolean",
    "Run provenance needs api_key_required boolean."
  );
  assertRepairMetadata(runInput);

  if (runInput.schema_version !== undefined) {
    requireValue(
      runInput.schema_version === "1.0.0",
      "Unsupported evaluated artifact schema version."
    );
  }
  const hasCaseId = runInput.case_id !== undefined;
  const hasCaseHash = runInput.case_sha256 !== undefined;
  requireValue(
    hasCaseId === hasCaseHash,
    "Embedded artifact identity must include both case_id and case_sha256."
  );
  const hasEvaluation = runInput.evaluation !== undefined;
  const hasAuditLog = runInput.audit_log !== undefined;
  const hasProvenanceVerification =
    runInput.provenance_verification !== undefined;
  if (hasProvenanceVerification) {
    requireValue(
      PROVENANCE_VERIFICATIONS.has(runInput.provenance_verification),
      "Artifact provenance_verification is not recognized."
    );
  }
  ...
  if (runInput.case_id !== undefined) {
    requireValue(
      runInput.case_id === caseData.id,
      `Embedded case_id ${runInput.case_id} does not match supplied case ${caseData.id}.`
    );
  }
  if (runInput.case_sha256 !== undefined) {
    requireValue(
      runInput.case_sha256 === caseFingerprint(caseData),
      "Embedded case_sha256 does not match the supplied case."
    );
  }
