...
- No physician fixture review or clinical validation was performed; clinical validation is not claimed. Never silently upgrade source review, deterministic checks, AI review, or a future scoped fixture review into clinical validation.
...
- The V2 release manifest contains `23/23` exact-byte artifacts. It includes reference evidence, three unchanged V1 lineage artifacts, and a fresh post-start Sol proof package. Current manifest SHA-256: `3e72fd8cf8edc0a00bbc513eb6bf498d24a550f8717d9df24d2837350f608541`.
...
- The first local post-start checkpoint is commit `21405c8e2d3e4d03e36442a1facab96c4ec487a6`, created `2026-07-13T13:15:43-05:00` on branch `codex/build-week`. It deliberately contains both the disclosed pre-start V1 lineage and the post-start extensions, so it is not independent proof that every checked-in file was created during Build Week. The entrant is submitting individually. Public MIT repository `https://github.com/Sammsamy/witnesspatch`, successful first-release exact-commit CI, and byte-verified deployment `https://witnesspatch.ankigpt.workers.dev` now exist; nothing has been submitted to Devpost.
- macOS is locally exercised. Linux verification is limited to the exact local Debian 12 arm64/Node 22.23.1 clean replay above; public GitHub Actions passes, while Windows remains unverified.
- The entrant is Fuzlullah Syed, submitting individually from the United States. Remaining human gates include `/feedback` Session ID, founder-voice YouTube demo, one healthcare-AI builder interview, final freeze, and Devpost completion. The authorized submission path does not claim physician review.
...
3. There is no completed builder interview, physician review, pilot, or public workflow baseline; the submission must explicitly say physician review was not performed.
...
6. Public source, exact first-release CI, logged-out deployment, and clean-browser verification exist. The final video, `/feedback` ID, exact submitted-commit freeze, external builder review, and Devpost submission do not.
