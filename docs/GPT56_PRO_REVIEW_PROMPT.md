...
- Licensed-physician fixture review is pending; clinical validation is not claimed. Never silently upgrade source review, deterministic checks, AI review, or a future scoped fixture review into clinical validation.
...
- The V2 release manifest contains `23/23` exact-byte artifacts. It includes reference evidence, three unchanged V1 lineage artifacts, and a fresh post-start Sol proof package. Current manifest SHA-256: `46542e562fe15b7fcec50f5a42d45e145614791c2bac7432ce5289440b9597f2`.
- `witnesspatch compile --case PATH --run PATH --out-dir PATH [--rule ID] [--fact-scope failure-prefix|full-trace]` independently regrades a supplied trace without invoking a model or target, selects the earliest failed critical contract by default, emits an atomic nine-file hash-listed witness bundle, and generates an executable regression that is red on the failed run and green when pointed at the repaired run. Its static reduction holds recorded decisions fixed; it is not target-in-the-loop or counterfactual minimization. For the two manifest-listed public inputs, only the explicit `failure-prefix` scope is claimed byte-identical to the browser.
...
- The Node verifier additionally validates the full proposal schema/semantics, recompiles the candidate, strictly applies the exact captured patch to the exact baseline and requires the exact candidate bytes, executes the retained JavaScript candidate on both V2 cases, and independently checks the exact four-holdout signature.
...
- Claim-hardened checkpoint `04ffd9feaac99b7e3c064cb19ba9159cfaec2fca` passed `npm ci` and `npm run verify:release` from clean clones on macOS 26.5.2 arm64/Node 24.14.0 and a local Debian 12 arm64 container/Node 22.23.1. macOS passed `125/125` engine tests; Debian passed `124` and expectedly skipped the one case-insensitive-filesystem test. Both passed `5/5` rendered-product tests, the `1/1` real development-server HTTP smoke, `5/5` submission-package checks, lint, typecheck, production static build, the distribution-license gate, byte-identical bundled third-party notices, and the 59-asset Wrangler dry run. Their physical 55-file `dist/client` snapshots matched byte-for-byte after normalizing only a platform-specific leading `./` in checksum-list paths; the SHA-256 of each canonical 55-line manifest was `31767c92a17ede21ef047d8b5774788e5ee3afefa96bf465739e322240dd9843`. Recheck the attached latest transcript; test source alone is not proof of a passing command.
...
- The current source-hash-bound V2 app has been exercised in a fresh production-Chrome session on both paths. The reference judge flow moved `50→100`, kept the baseline visibly red, verified `23/23` artifacts, freshly regraded the reference `2/2`, reran `4/4` reference holdouts, showed fresh Sol JSON-IR replay at `2/2 + 4/4`, and displayed the four-case baseline/repair/control/mutant closure. The local-input path loaded the included pair and produced a nine-file `50/100` red bundle with `2` hashes computed and `0` externally verified, exact CLI parity, red/green executable closure, and no requests after the two expected sample fetches. Both recorded zero console errors and zero warnings; same-page controls produced no `/.rsc` or `404` requests.
- The first local post-start checkpoint is commit `21405c8e2d3e4d03e36442a1facab96c4ec487a6`, created `2026-07-13T13:15:43-05:00` on branch `codex/build-week`. It deliberately contains both the disclosed pre-start V1 lineage and the post-start extensions, so it is not independent proof that every checked-in file was created during Build Week. Nothing has been pushed, deployed, or submitted; there is no remote. The selected release route is a public static demo plus an all-rights-reserved private GitHub repository shared with `testing@devpost.com` and `build-week-event@openai.com`; repository creation, sharing, and judge-equivalent access verification remain outstanding.
- macOS is locally exercised. Linux verification is limited to the exact local Debian 12 arm64/Node 22.23.1 clean replay above; remote CI and Windows are unverified.
- Remaining human gates include both teammates' eligibility/registration and representative choice, `/feedback` Session ID, private-repository creation and judge-sharing verification, live deployment, founder-voice YouTube demo, one healthcare-AI builder interview, and one licensed-physician review.
...
The internal planning scorecard currently says **30.5/40**. Treat that as an untrusted local hypothesis, not a prior: independently score the attached evidence and do not average, round toward, or repeat it without justification.
...
6. The public app, remote GitHub Actions CI visible inside the judge-shared private repository, verified judge access, final video, `/feedback` ID, and submission do not yet exist.
