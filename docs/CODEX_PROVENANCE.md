...
The official submission also requires the Codex Session ID returned after `/feedback` is submitted for the task where most core functionality was built. That identifier has not yet been added to this record; the technical task/thread UUID is not a substitute.
...
Exact candidate `87dee97` passed `npm ci` and `npm run verify:release` from fresh clones on macOS 26.5.2 arm64/Node 24.14.0 and a local Debian 12 arm64 container/Node 22.23.1. macOS passed all `158/158` aggregate test executions; Debian passed `157/158`, with only the expected case-insensitive-filesystem test skipped. Both passed `136` discovered core tests, `6/6` rendered-product checks, `1/1` real development-server HTTP smoke, `9/9` submission-package checks, `6/6` deployment-rendered checks, the zero-argument judge proof, detached-bundle verification coverage, the exact threat-model boundary check, build, lint, typecheck, the distribution-license gate, byte-identical bundled third-party notices, and the 59-asset Wrangler dry run. Their physical 55-file `dist/client` snapshots matched byte-for-byte with canonical manifest SHA-256 `1133339d1b372491084a06f889acd97399f2de2988d6267eb13baa48dd4b3721`. The source-hash-bound production candidate separately passed both the manifest-bound reference compile/verify and included-sample local compile/export; the local ZIP matched the CLI byte-for-byte across nine files and executed red by default and green with the supplied repair. The workflow now promotes the six-line proof to the GitHub job summary and retains it as a release artifact. The final submitted commit still needs remote CI and a final replay after external fields freeze. The Linux claim remains limited to the exact local container; Windows is unverified.
