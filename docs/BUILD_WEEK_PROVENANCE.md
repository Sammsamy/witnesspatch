...
The feature-frozen production-Chrome desktop replay passed both paths. The reference flow recorded `50 → 2/2 exact manifest inputs → nine-file BASELINE RED → 100`, `23/23` retained responses, both proof signatures, and explicit `baseline RED · retained repair PASS`. The isolated local-input flow freshly produced `50/100`, `INV-02` and `INV-03`, `9→3` facts, nine files, and `2 hashes computed · 0 externally verified`, with no post-load requests. The local ZIP passed `unzip -t`, all nine files were byte-identical to the shipped CLI output, its default regression exited red (`1`), and the supplied repaired candidate exited green (`0`). Both flows recorded zero console warnings and zero console errors. An exact 390 x 844 mobile addendum covered the fresh hero, compiled local result, and verified receipt with document and body widths fixed at 390, no horizontal overflow, required controls visible, and zero console warnings or errors.
...
- Release-code checkpoint `214854a52e7e8f5d6265405881b80a1965b5c0f5` passes `npm ci` followed by `npm run verify:release` from clean clones on macOS 26.5.2 arm64 with Node 24.14.0/npm 11.9.0 and a local Debian 12 arm64 container with Node 22.23.1/npm 10.9.8. macOS passes `125/125` core tests; Debian passes `124` and expectedly skips the one case-insensitive-filesystem test on its case-sensitive filesystem. Both pass `5/5` rendered tests, the `1/1` real development-server HTTP smoke, `5/5` submission-package checks, build, lint, typecheck, the distribution-license gate, byte-identical bundled third-party notices, and a 59-asset Wrangler dry run. Their physical 55-file `dist/client` snapshots are byte-identical; the SHA-256 of the matching 55-line file-checksum manifest is `221d4ce0abf95accd6d8b73ec58fec3eaadec3ae0c52fc864984ff4382b26bd0`.
