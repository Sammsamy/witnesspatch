...
- The feature-frozen working tree passes `npm run verify:release`: `125/125` core tests, `5/5` rendered tests, a `1/1` real development-server HTTP smoke, `5/5` submission-package checks, build, lint, typecheck, the 624-package/15-static-package distribution-license gate, byte-identical deployed notices, and a 59-file Wrangler dry run. These are local macOS working-tree results; clean replay of the new commit, Linux, and Windows are not yet verified for this exact profile.
- Submission-package checkpoint `b4c8217fd93ac764d9d48bb72e8f105559fc3782` passed `npm ci` followed by the full expanded release verifier from a fresh local clone on macOS with Node 24.14.0, including the first-page and retained-manifest HTTP smoke and `4/4` submission-package checks. Checkpoints `9f2cd8da2ea03f294a247cb91795571e744a68fb`, `15f1aee67b419822cab54310ae9bec7cd321cb9c`, and `a056fe337208c0f377918bb8502d2bc6324ba19a` preserve the earlier release and cross-runtime evidence. See `docs/CLEAN_CHECKOUT_RECEIPT.md`; an exact submitted-commit replay after final freeze, Linux, Windows, remote GitHub Actions CI visible inside the judge-shared private repository, and independent external reproduction remain outstanding.
