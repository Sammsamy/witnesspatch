...
The current source-hash-bound production-Chrome desktop replay passed both paths. The reference flow recorded `50 → 2/2 exact manifest inputs → nine-file BASELINE RED → 100`, `23/23` retained responses, both proof signatures, explicit `baseline RED · retained repair PASS`, and the four-case baseline/repair/control/mutant closure. The isolated local-input flow loaded the included pair and freshly produced `50/100`, `INV-02` and `INV-03`, `9→3` facts, nine files, and `2 hashes computed · 0 externally verified`; after the two expected same-origin JSON fetches, compilation made no further requests. The local ZIP passed `unzip -t`, all nine files were byte-identical to the shipped CLI output, its default regression exited red (`1`), and the supplied repaired candidate exited green (`0`). Both flows recorded zero console warnings and zero console errors, and the same-page controls produced no `/.rsc` or `404` requests. Exact 390 x 844 reference and local replays had document widths fixed at 390, no horizontal overflow, required controls visible, and zero console warnings or errors.
...
- Claim-hardened checkpoint `04ffd9feaac99b7e3c064cb19ba9159cfaec2fca` passes `npm ci` followed by `npm run verify:release` from clean clones on macOS 26.5.2 arm64 with Node 24.14.0/npm 11.9.0 and a local Debian 12 arm64 container with Node 22.23.1/npm 10.9.8. macOS passes `125/125` core tests; Debian passes `124` and expectedly skips the one case-insensitive-filesystem test on its case-sensitive filesystem. Both pass `5/5` rendered tests, the `1/1` real development-server HTTP smoke, `5/5` submission-package checks, build, lint, typecheck, the distribution-license gate, byte-identical bundled third-party notices, and a 59-asset Wrangler dry run. Their physical 55-file `dist/client` snapshots match byte-for-byte after normalizing only the platform-specific leading `./` in checksum-list paths; the SHA-256 of each canonical manifest is `31767c92a17ede21ef047d8b5774788e5ee3afefa96bf465739e322240dd9843`.
- Submission-package checkpoint `b4c8217fd93ac764d9d48bb72e8f105559fc3782` passed the earlier full release verifier from a fresh local macOS clone; checkpoints `9f2cd8da2ea03f294a247cb91795571e744a68fb`, `15f1aee67b419822cab54310ae9bec7cd321cb9c`, and `a056fe337208c0f377918bb8502d2bc6324ba19a` preserve earlier release and cross-runtime evidence. See `docs/CLEAN_CHECKOUT_RECEIPT.md`. An exact submitted-commit replay after the external release fields are frozen, Windows verification, remote GitHub Actions CI visible inside the judge-shared private repository, and independent external reproduction remain outstanding; the Linux claim is limited to the exact local Debian container above.
