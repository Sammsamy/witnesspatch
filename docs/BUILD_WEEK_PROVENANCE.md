...
- Release-code checkpoint `15f1aee67b419822cab54310ae9bec7cd321cb9c` passed `npm ci` followed by the full release verifier from a fresh local clone on macOS with Node 24.14.0: `104/104` core tests, `5/5` rendered tests, the distribution-license gate, and the 56-file Wrangler dry run. Checkpoint `a056fe337208c0f377918bb8502d2bc6324ba19a` also passed the same path from fresh local clones on Node 22.15.0 and Node 24.14.0. See `docs/CLEAN_CHECKOUT_RECEIPT.md`; an exact submitted-commit replay after final freeze, Linux, Windows, public CI, and independent external reproduction remain outstanding.
