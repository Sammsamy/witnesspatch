...
- The July 13 request used the entrant's Devpost-account address, while the entrant later confirmed a different address is tied to the intended ChatGPT/Codex account. The FAQ says an awarded balance appears under `chatgpt.com` **Settings → Usage** for the account used to request it, so mapping remains unresolved. Correct the existing response through an organizer-enabled edit link or official support if possible; do not submit a duplicate. Keep both addresses out of the repository and keep Auto top-up disabled. The build and judge path remain independent of receiving credits and work without incremental spend.
