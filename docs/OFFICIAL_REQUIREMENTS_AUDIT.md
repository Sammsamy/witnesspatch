...
| README with setup, sample data if needed, and run guidance | Release-code checkpoint `214854a` passed the full verifier from clean macOS and local Debian clones, including HTTP smoke, `5/5` submission checks, and byte-identical static snapshots | Preserve `docs/CLEAN_CHECKOUT_RECEIPT.md` and rerun the exact submitted commit after final freeze. This does not replace the required external no-rebuild judge path. |
