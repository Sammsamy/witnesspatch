...
| Code repository for judging and testing | Not yet created; current checkout is `UNLICENSED` | Obtain ownership/license authorization. The standing completion target is a public repository with a detected open-source license and logged-out source/CI access. The final freeze now requires a successful completed public `Verify` push run for the exact default-branch commit. The rules-compliant fallback is a private repository shared with `testing@devpost.com` and `build-week-event@openai.com`, but it does not satisfy this task's stricter public-repository goal. |
| README with setup, sample data if needed, and run guidance | Exact candidate `87dee97` passed the full verifier from fresh macOS and local Debian clones, including the judge proof, HTTP smoke, `9/9` submission checks, threat-model boundary check, and byte-identical 55-file static snapshots with canonical manifest SHA-256 `1133339d1b372491084a06f889acd97399f2de2988d6267eb13baa48dd4b3721` | Preserve `docs/CLEAN_CHECKOUT_RECEIPT.md` and rerun the exact submitted commit after final freeze. This does not replace the required external no-rebuild judge path. |
...
- The current `UNLICENSED` package posture is consistent only with the private judge-shared fallback. The July 13 official update says a public repository requires a relevant open-source license, so the standing public goal requires a deliberate entrant-approved license decision plus the existing third-party notices; the rules do not prescribe a specific license.
