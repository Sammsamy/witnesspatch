...
| README with setup, sample data if needed, and run guidance | Release-code checkpoint `15f1aee` passed from a fresh local clone; earlier cross-runtime clones passed at `a056fe3` | Preserve `docs/CLEAN_CHECKOUT_RECEIPT.md` and rerun the exact submitted commit after final freeze. This does not replace the required external no-rebuild judge path. |
