...
|---|---|---|
| Registration: July 9 at 10:00 AM through July 21 at 5:00 PM PT | Creator is joined; no teammate is currently listed | Add a teammate only if that eligible person actually joins and accepts before the deadline; otherwise submit truthfully as an individual. |
...
| Code repository for judging and testing | Public creation is authorized; the checkout now uses the MIT license, but no remote exists yet | Create `Sammsamy/witnesspatch`, push the exact release commit, confirm GitHub detects the license, and verify source plus CI from a logged-out request. The final freeze requires a successful completed public `Verify` push run for the exact default-branch commit. |
