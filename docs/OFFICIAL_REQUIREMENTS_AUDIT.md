...
| README with setup, sample data if needed, and run guidance | Passed from clean local clones at checkpoint `a056fe3` | Preserve `docs/CLEAN_CHECKOUT_RECEIPT.md` and rerun the exact final submitted commit. This does not replace the required external no-rebuild judge path. |
...
| Authenticated submission fields | Not inspected | The public rules enumerate the category, description, video, repository, and `/feedback` ID, but additional authenticated labels or limits are unverified. Inspect the joined challenge form without submitting and record them in `docs/SUBMISSION_FORM_CHECKLIST.md`. |
