...
Rechecked July 20, 2026 against the live [Devpost overview](https://openai.devpost.com/), [resources page](https://openai.devpost.com/resources), [FAQ](https://openai.devpost.com/details/faqs), [schedule](https://openai.devpost.com/details/dates), controlling [Official Rules](https://openai.devpost.com/rules), [OpenAI Build Week page](https://openai.com/build-week/), and [OpenAI Usage Policies](https://openai.com/policies/usage-policies/). The rules prevail over conflicting hackathon materials.

## Confirmed requirements

| Requirement | Current state | Action |
|---|---|---|
| Registration: July 9 at 10:00 AM through July 21 at 5:00 PM PT | Creator is joined and has authorized an individual submission; no teammate is listed | Keep the submission individual and do not introduce a teammate claim. |
...
| Code repository for judging and testing | [Public MIT repository](https://github.com/Sammsamy/witnesspatch) exists; logged-out API access confirms public visibility, default-branch commit `4d27f39`, MIT detection, and successful public [Verify run](https://github.com/Sammsamy/witnesspatch/actions/runs/29772464327) | Preserve public access and rerun exact-commit CI after the final video/feedback metadata freeze. |
| README with setup, sample data if needed, and run guidance | Exact candidate `87dee97` passed the prior full verifier from fresh macOS and local Debian clones. The current authorized candidate retains a 55-file static shape with locally gated canonical manifest SHA-256 `523aeaa83f9028036b258b192c989bc5a10fbba9b21280273bb1b2283fc744d5` | Preserve `docs/CLEAN_CHECKOUT_RECEIPT.md`, run the current full verifier, then verify exact-commit public CI and rerun the exact submitted commit after final freeze. This does not replace the required external no-rebuild judge path. |
...
| Developer tools need installation instructions, supported platforms, and a no-rebuild judge test path | Pass for current public checkpoint | Node 22.15+ setup, one-command judge proof, included synthetic sample, public static replay, and deterministic verifier are documented. Do not broaden the exact local Debian evidence into a Linux browser matrix or Windows claim. |
| Free working access during judging | [Live static replay](https://witnesspatch.ankigpt.workers.dev) and repository are publicly reachable without charge or login | Keep both available without restriction through at least August 9 at 5:00 PM PDT. The clean public reference compile/verify passed with zero console warnings or errors and 53/53 served-file byte equality. |
...
| Authenticated submission fields | Rechecked and edited with authorization July 20: draft `1079808` lists only Fuzlullah Syed and remains `2/5`; title, pitch, exact story, 22 normalized tags, live/repository links, individual/U.S./Developer Tools selections, repository, judge-testing path, and developer-tool instructions are saved. Thumbnail/gallery upload, public video, and `/feedback` remain blank | Enable Chrome local-file access for the reviewed PNGs, add the real public video and Codex-returned `/feedback` ID, then audit again. The final agreement remains unchecked and final submission still requires separate user approval. |
...
- Registered and eligible entrants could optionally request $100 in Codex credits by July 17 at noon PDT. The form described requests as first-come, first-served, not guaranteed, and limited to one code per entrant. As of July 20, the official resources page says all available credits have been given out.
- Event credits are strictly for the submission, have no cash value, and, under the controlling rules, must be used by July 21 at 5:00 PM PT. The FAQ says the event supplies Codex credits only—not separate API credits or tokens. OpenAI's general [Service Credit Terms](https://openai.com/policies/service-credit-terms/) also prohibit transferring, selling, or exchanging service credits.
- The July 13 request used the entrant's Devpost-account address, while the entrant later confirmed a different address is tied to the intended ChatGPT/Codex account. An official event-manager clarification said the request-form email must match Devpost registration and that the receiving ChatGPT/Codex account may use a different email, so the submitted form identity was correct. Do not submit a duplicate. Because the Resources page now says all credits are exhausted, treat delivery as unavailable unless a previously issued redemption message or Settings → Usage balance proves otherwise. Keep both addresses out of the repository and keep Auto top-up disabled. The build and judge path remain independent of receiving credits and work without incremental spend.
...
- preserves canonical machine-readable `fixture_wording_review: not_performed` and `clinical_validation: not_claimed` boundaries, while the legacy per-artifact `clinician_validation: pending` field remains solely for schema and provenance compatibility and is not treated as either canonical state; the UI explicitly displays that physician fixture/wording review was not performed and clinical validation is not claimed.
...
- The entrant authorized the public route and MIT licensing. `LICENSE`, `package.json`, `package-lock.json`, and `THIRD_PARTY_NOTICES.md` now agree on that scope; GitHub license detection and logged-out source access must still be verified after publication.
- `npm run licenses:check` gates the 624-entry locked dependency graph against a reviewed SPDX-expression inventory. The current private-source/public-static-demo distribution scope is audited; adding a project license or changing the route, packaging, or dependencies requires the full distribution and release gates again.
