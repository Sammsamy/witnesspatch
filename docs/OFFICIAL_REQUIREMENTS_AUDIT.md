...
| Authenticated submission fields | Rechecked read-only July 20: draft `1079808` is still `Untitled`, `1/5` steps complete, all project/additional-info fields are blank, and only its creator is listed | Follow `docs/SUBMISSION_FORM_CHECKLIST.md`: complete the required project story, tags, category, country, repository URL, public video, `/feedback` ID, and final rules checkbox only after every linked artifact and human gate is verified. Do not claim a teammate who has not joined. |
