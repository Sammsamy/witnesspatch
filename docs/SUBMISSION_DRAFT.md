...
- Local release verification: claim-hardened checkpoint `04ffd9f` passed `npm ci` and `npm run verify:release` from clean clones on macOS 26.5.2 arm64/Node 24.14.0 and a local Debian 12 arm64 container/Node 22.23.1. macOS passed `125/125` core tests; Debian passed `124` and expectedly skipped one case-insensitive-filesystem test. Both passed `5/5` rendered-product tests, the `1/1` real development-server HTTP smoke, `5/5` submission-package checks, build, lint, typecheck, the distribution-license gate, byte-identical bundled third-party notices, and a 59-asset Wrangler dry run; their physical 55-file `dist/client` snapshots matched byte-for-byte after normalizing only a checksum-list path prefix.
