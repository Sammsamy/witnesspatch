...
| **Adversarial total** | **29.2 / 40** | **34.7 / 40** | The project is technically credible but not winner-safe. Public release gates are mandatory; external workflow evidence is the largest scoring lever that local testing cannot manufacture. |
...
- [x] Pass exact release candidate `ff5ce1f` through fresh macOS and Debian clones after judge-proof and detached-bundle hardening: `157/157` aggregate tests on macOS, `156/157` plus one expected filesystem skip on Debian, zero vulnerabilities, and the unchanged canonical 55-file static fingerprint.
