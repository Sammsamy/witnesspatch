...
The July 20 late-window adversarial rescore is `29.0/40`, with an honest range of `27.8–30.4/40` **if** all mandatory external submission gates are completed. The detached-bundle verifier, six-line judge proof, sharper positioning, corrected receipt frame, compliant local narration fallback, CI summary, threat model, exact-commit release gate, and full public-deployment byte gate strengthen execution. Newly visible public Build Week competitors—especially Ouroboros, OutcomeLoop, and Remnic Relay—make generic repair, independent-verifier, red/green closure, judge-package, and receipt claims less distinctive, holding the idea estimate at `6.6`. As submitted today, the project would fail before Stage Two because repository access, deployment, public video, `/feedback` ID, and the Devpost entry are missing. The public gallery remains unpublished, so these repositories are competitive-risk evidence rather than confirmed eligible submissions. The table is a decision aid, not a measured judge prediction.
...
- [ ] Apply the authorized MIT license, create the public repository, and verify the exact default-branch commit plus CI from a logged-out account. The final freeze will reject any commit without a successful completed public `Verify` push run.
