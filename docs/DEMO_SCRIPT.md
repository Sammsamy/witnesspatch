...
|---|---|---|
| `0:00–0:13` | “I’m a third-year medical student. Healthcare-agent failures die as screenshots. WitnessPatch records what the agent knew, then turns a rule-breaking answer into a failing test.” | Start at `#top`. Keep `50`, `2` critical breaches, `T+02`, and **Locked rules** visible. |
...
| `1:00–1:14` | “It pinpoints INV-02 at T-plus-two and narrows nine facts to three with the response fixed. The exported test stays red; the repair is checked separately.” | Show `2/2 exact inputs`, `9 files`, `INV-02`, `T+02`, `9→3`, generated `regression.test.mjs`, and **BASELINE RED**. Select **Export complete 9-file ZIP** and let its label change. |
| `1:14–1:38` | “But this is not just a canned replay. Compile Your Files accepts a synthetic case and failed run, hashes and regrades them only in this browser, labels them zero externally verified, and exports the same nine-file format.” | Return to `#top` with **WitnessPatch** and select **Compile your files**. Upload `cases/v2/postpartum-warning-signs.json` and `engine/fixtures/v2/postpartum-warning-signs-baseline.input.json`, check the fully synthetic confirmation, and select **Compile red witness**. Show **COMPILED RED**, `2 hashes computed · 0 externally verified`, and `9 files`; then select **Return to reference demo**. |
| `1:38–1:55` | “Back on the reference, the verifier checks 23 saved files, recalculates both scores, and runs four challenges. Same case and rules: 50 becomes 100 with no critical failures.” | Select **Verify retained repair** and wait for **Freshly verified in this browser**. Hold on `100`, `0`, and **Contract satisfied**. |
...
| `2:37–2:53` | “No patient data, API key, or live model call. Blood-pressure labels come from the synthetic fixture. Physician review is pending; this is developer tooling, not clinical use or safety certification.” | End on the receipt with **None · fully synthetic**, **API key Not required**, BP scope, **Licensed-physician fixture review pending**, and **Clinical use Not permitted** visible. |
