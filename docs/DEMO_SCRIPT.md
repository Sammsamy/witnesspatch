...
| `1:14–1:38` | “But this is not just a canned replay. Compile Your Files accepts a synthetic case and failed run, hashes and regrades them only in this browser, labels them zero externally verified, and exports the same nine-file format.” | Return to the top with **WitnessPatch** and select **Compile your files**. Select **Load included sample**, wait until both loaded filenames appear, check the fully synthetic confirmation, and select **Compile red witness**. Show **COMPILED RED**, `2 hashes computed · 0 externally verified`, and `9 files`; then use the persistent header **Reference demo** control followed by **WitnessPatch** to reset the reference view. |
