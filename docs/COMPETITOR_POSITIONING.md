...
| [Ouroboros](https://github.com/wlsalswo14/Ouroboros) | Public Build Week Developer Tools candidate exports evaluator-owned failure reports for Codex, validates one allowlisted transformation at a time, executes a candidate capability graph, and uses a blind evaluator plus certificate for promotion | Independent repair evaluation is not WitnessPatch's differentiator. WitnessPatch first turns one already-observed trace-level missed deadline into a portable conventional red test using only facts available at that deadline; the red handoff exists before any separate repair is considered |
| [OutcomeLoop](https://github.com/tinyopsstudio/outcomeloop-build-week) | Public Build Week Developer Tools candidate runs and resumes a Codex session until a protected external verifier passes, isolates agent and verifier execution, fingerprints protected inputs, and signs completion receipts | WitnessPatch does not claim external-verifier authority, protected completion loops, or signed receipts as novel. Its object is a recorded behavioral failure: compile the earliest missed action deadline into a portable offline regression and fixed-decision temporal witness |
| [Remnic Relay](https://github.com/joshuaswarren/remnic) | Public Build Week extension exposes the stale memory behind an agent failure, gates a correction on human approval, starts a fresh agent, requires the same test to turn green, and binds the run, UI, fixture, and receipt into a dependency-free judge package | WitnessPatch does not claim human-approved correction, fresh-agent propagation, red/green closure, or an evidence-bound judge package as novel. It compiles an already-recorded missed action deadline from only the facts available at that deadline into a conventional red test before any correction is evaluated |
| [PatchPilot](https://github.com/DmytroHuzz/patchpilot) | Public Build Week Developer Tools submission uses deterministic vulnerability facts, bounded GPT-5.6 interpretation, approval-gated isolated patching, tests, rescan, and an evidence handoff | WitnessPatch is not a general remediation orchestrator. It compiles a failed time-fenced agent trace into a reusable test bundle; the retained target patch is only a separate proof that the exported regression can close without breaking the exact authored control |
| [Sieve](https://github.com/aruneem-bhowmick/sieve) | Public Build Week candidate intervenes on structured coding-agent rationales and measures whether patches and tests change, with an explicit causal-faithfulness boundary | WitnessPatch does not infer rationale faithfulness or model internals. It evaluates declared observable actions against authored deadlines and preserves the exact observed miss as an executable regression |
| [LineageReceipt](https://github.com/Yesol-Pilot/lineage-receipt) | Public Build Week Developer Tools candidate blocks incomplete ML releases from DataHub lineage, hashes a canonical evidence snapshot and gap IDs, and writes the deterministic decision provenance back to DataHub | WitnessPatch does not claim evidence receipts, SHA-bound release decisions, or fail-closed release gates as novel. Its compiled object is the earliest failed time prefix and a portable executable regression, not an ML-lineage approval record |
