...
> Trace dashboards, deterministic replayers, and failure-to-dataset eval tools already exist. WitnessPatch starts from a known failure plus an authored action contract, emits a standalone standard-test bundle, then closes the handoff with time-fenced facts, a bounded static witness, an inspectable repair, an exact-fact control, mutation checks, and byte-identical browser/Node artifacts.
...
| [Promptfoo agent trajectory testing](https://www.promptfoo.dev/docs/red-team/agents/) | Time-ordered agent trajectories, tool/order/budget assertions, red-team findings promoted into targeted CI regressions, and JUnit output | Does not document this exact known-failure-to-standalone-bundle handoff or the time-fact witness plus repair/control/mutation/byte-parity closure |
