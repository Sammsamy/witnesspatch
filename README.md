...
| --- | --- | --- |
| Source install and release verification | Clean-clone commit `214854a` passes the complete verifier on macOS 26.5.2 arm64/Node 24.14.0 and local Debian 12 arm64/Node 22.23.1; both yield the same physical 55-file `dist/client` byte manifest and pass the 59-asset Wrangler dry run | Verified only for those exact local environments; rerun the exact submitted commit after final form freeze |
