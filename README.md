...
| --- | --- | --- |
| Source install and release verification | Fresh-clone commit `ff5ce1f` passes the complete verifier on macOS 26.5.2 arm64/Node 24.14.0 and local Debian 12 arm64/Node 22.23.1; both yield the same canonical physical 55-file `dist/client` byte manifest and pass the 59-asset Wrangler dry run | Verified only for that exact candidate and those exact local environments; remote CI and an exact submitted-commit replay remain |
